...
const temp = fs.readFileSync("metaDataLookup.json"); //json file which contain the lookup values of codes to their respective values
  ...
  try {
    ...
    do {
      const records = await client.search(
        resource,
        className,
        `(MLSListDate=${getTodayDate()}) AND (ModificationTimestamp=${formattedTime}+)`,
        {
          limit: 500,
          offset,
